...
#### `dev_auth`
Аутентификация для разработческого клиента. Отправляется вместо `auth`. Позволяет подключиться без регистрации и пароля.
- **Направление**: C -> S
- **Поля**:
  - `type: "dev_auth"`
  - `name: str` - Желаемое имя игрока.
  - `uuid: str` - Уникальный идентификатор клиента (может быть сгенерирован на лету).
...
## 8. Функции для разработки

Для упрощения тестирования и разработки плагинов предусмотрены специальные инструменты.

### Конфигурация сервера (`server_config.json`)

В корне проекта находится файл `server_config.json`, который позволяет настраивать параметры сервера.
...
- `host` (str): IP-адрес, на котором будет запущен сервер.
- `port` (int): Порт для прослушивания.
- `tick_rate` (int): Количество игровых тактов в секунду.
- `allow_dev_client` (bool): Если `true`, сервер будет принимать подключения от разработческих клиентов (`dev_client.py`) без необходимости регистрации.

### Клиент для разработки (`dev_client.py`)

Это полноценный игровой клиент, который позволяет быстро подключиться к серверу, минуя экраны логина и регистрации. Основная цель — тестирование геймплея, плагинов и сетевого взаимодействия.

**Запуск:**
```bash
python dev_client.py --name <ИмяИгрока> [--uuid <UUID>]
```
- `--name`: Устанавливает имя для вашего персонажа. По умолчанию "DevPlayer".
- `--uuid`: (Опционально) Позволяет задать конкретный UUID для клиента. Если не указан, будет сгенерирован случайный UUID при каждом запуске.

Это позволяет запускать несколько клиентов с одного IP-адреса, что удобно для отладки мультиплеерных механик.

### CLI-клиент для разработки (`dev_cli_client.py`)

Для "headless" тестирования (без графического интерфейса) существует `dev_cli_client.py`. Он подключается к серверу и позволяет отправлять и получать сообщения через консоль.

**Запуск:**
```bash
python dev_cli_client.py --name <ИмяИгрока>
```
- `--name`: Устанавливает имя для вашего персонажа. По умолчанию "DevPlayer".

Этот клиент полезен для быстрой проверки сетевых пакетов или отладки серверной логики без необходимости запускать тяжелый графический клиент.