...
class AudioIntegration(PluginModule):
    ...
    def on_load(self):
        ...
        # Автозапуск музыки при входе в игру
    ...
    def _init_audio_manager(self):
        ...
        try:
            ...
            if hasattr(self.app, 'audio_manager') and self.app.audio_manager:
                ...
                self.logger.info("Using existing AudioManager")
            else:
    ...
    def _on_game_state_changed(self, data: dict):
        ...
        try:
            ...
            if new_state == GameState.IN_GAME:
                self._start_default_music()
        except Exception:
            # Fallback - проверяем по строке
            state_str = str(new_state) if new_state else ""
            if "IN_GAME" in state_str:
                self._start_default_music()

    def _start_default_music(self):
        """Запускает фоновую музыку по умолчанию."""
        if not self.audio:
            self._init_audio_manager()

        if self.audio:
            self.logger.info("Starting default music...")
            self.audio.play_bgm("adventure")
            self.audio.set_ambient("forest_day")
            self.logger.info("Default music started: adventure + forest_day ambient")
    ...
    def play_ui_sound(self, sound_name: str):
        ...
        if self.audio:
            method = getattr(self.audio, f"play_ui_{sound_name}", None)
            if method:
                method()
