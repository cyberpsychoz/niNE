...
1. **`nine/core/entity.py`** - Компонентная система для статичных игровых объектов (предметы, инвентарь)
2. **`nine/core/ecs.py`** - Entity Component System для динамических сущностей (игроки, NPC)

Обе системы сосуществуют намеренно и решают разные задачи.

> **Март 2026:** ECS унифицирована — игроки и NPC в одном `PooledECSWorld`. Все компоненты в `nine/core/components.py`. NPC плагин (`sh_components.py`) стал re-export shim.
...
- Предметы (Items) — 112+ зарегистрированных entity (оружие, броня, аксессуары, бытовые предметы)
- Инвентарь (Inventory) — стартовое снаряжение автоматически выдаётся при создании персонажа
